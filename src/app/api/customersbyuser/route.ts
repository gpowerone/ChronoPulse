...
export async function GET(request, { params }) {
  ...
  try {
    // Use authenticated user's ID from session (NOT from query parameter)
    // This ensures users can only access their own customer profile
    const query = `
      SELECT 
    ...
    const result = await db_query(db, query, [user_id]);
    ...
    return NextResponse.json(
      { error: 'Failed to retrieve customer profile', message: error.message },
      { status: 500 }
    );
  }
}
